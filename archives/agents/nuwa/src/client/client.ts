...
export class A2AClient {
  ...
  private async _handleJsonResponse<Res extends JSONRPCResponse>(
    ...
    try {
      ...
      // Check if result exists, which is required for successful responses
      if (!('result' in jsonResponse) && !jsonResponse.error) {
           throw new RpcError(
               -32603, // Internal Error
               "Invalid JSON-RPC response: Successful response is missing 'result' field."
           );
      }

      // Extract and return only the result payload (which might be undefined or null)
  ...
  sendTaskSubscribe(
    ...
    // Wrap the raw generator to filter out null/undefined results
    const rawGenerator = async function* (this: A2AClient) {
      const httpResponse =
      ...
      // Yield potentially null/undefined results
    ...
    return (async function* () {
      for await (const event of rawGenerator) {
        if (event !== null && event !== undefined) {
          yield event as TaskStatusUpdateEvent | TaskArtifactUpdateEvent;
        }
      }
    })();
  }
