...
describe('CryptoUtils', () => {
  ...
  describe('derivePublicKey', () => {
    it('should derive Ed25519 public key from private key', async () => {
      const keyPair = await CryptoUtils.generateKeyPair(KeyType.ED25519);
      const derivedPublicKey = await CryptoUtils.derivePublicKey(keyPair.privateKey, KeyType.ED25519);
    ...
    it('should derive Secp256k1 public key from private key', async () => {
      const keyPair = await CryptoUtils.generateKeyPair(KeyType.SECP256K1);
      const derivedPublicKey = await CryptoUtils.derivePublicKey(keyPair.privateKey, KeyType.SECP256K1);
    ...
    it('should derive EcdsaR1 public key from private key', async () => {
      const keyPair = await CryptoUtils.generateKeyPair(KeyType.ECDSAR1);
      const derivedPublicKey = await CryptoUtils.derivePublicKey(keyPair.privateKey, KeyType.ECDSAR1);
  ...
  describe('provider exception handling', () => {
    it('should reject CryptoKey for Secp256k1 signing', async () => {
      ...
      await expect(
        CryptoUtils.sign(data, privateKey, KeyType.SECP256K1)
      ).rejects.toThrow('CryptoKey is not supported for Secp256k1 signing');
    });
  ...
  describe('WebAuthn algorithm mapping', () => {
    it('should map WebAuthn algorithms to KeyType correctly', () => {
      const { algorithmToKeyType, keyTypeToAlgorithm, getSupportedAlgorithms } = require('../../src/types/crypto');
  ...
  describe('crypto provider factory', () => {
    it('should throw error for unsupported key type', async () => {
      await expect(
        CryptoUtils.generateKeyPair('UnsupportedKeyType2024' as any)
    ...
    it('should throw error for unsupported provider creation', () => {
      ...
      expect(() => 
        defaultCryptoProviderFactory.createProvider('UnsupportedType' as any)
      ).toThrow('No provider available for key type');
    });
  });
  ...
  describe('edge cases and error handling', () => {
    it('should handle empty data signing and verification', async () => {
      ...
      const isValid = await CryptoUtils.verify(emptyData, signature, keyPair.publicKey, KeyType.ED25519);
      expect(isValid).toBe(true);
    });
    ...
    it('should handle corrupted signature gracefully', async () => {
      ...
      const isValid = await CryptoUtils.verify(data, corruptedSignature, keyPair.publicKey, KeyType.ED25519);
      expect(isValid).toBe(false);
  ...
  describe('Rooch signature scheme conversion', () => {
    ...
    it('should throw error for unsupported KeyType to Rooch conversion', () => {
      ...
      expect(() => 
        keyTypeToRoochSignatureScheme('UnsupportedType' as any)
    ...
    it('should throw error for unsupported Rooch scheme conversion', () => {
      ...
      expect(() => 
        roochSignatureSchemeToKeyType('UnsupportedScheme' as any)
      ).toThrow('Unsupported Rooch signature scheme');
    });
  });
  ...
  describe('Additional edge cases', () => {
    it('should handle very large data for signing', async () => {
      ...
      const isValid = await CryptoUtils.verify(largeData, signature, keyPair.publicKey, KeyType.ED25519);
      expect(isValid).toBe(true);
    });
    ...
    it('should handle zero-filled data', async () => {
      ...
      const isValid = await CryptoUtils.verify(zeroData, signature, keyPair.publicKey, KeyType.SECP256K1);
      expect(isValid).toBe(true);
    });
    ...
    it('should handle single byte data', async () => {
      ...
      const isValid = await CryptoUtils.verify(singleByte, signature, keyPair.publicKey, KeyType.ECDSAR1);
      expect(isValid).toBe(true);
    });
    ...
    it('should produce different signatures for different data with same key', async () => {
      ...
      expect(await CryptoUtils.verify(data1, signature1, keyPair.publicKey, KeyType.ED25519)).toBe(true);
      expect(await CryptoUtils.verify(data2, signature2, keyPair.publicKey, KeyType.ED25519)).toBe(true);
      
      // Cross-verification should fail
      expect(await CryptoUtils.verify(data1, signature2, keyPair.publicKey, KeyType.ED25519)).toBe(false);
      expect(await CryptoUtils.verify(data2, signature1, keyPair.publicKey, KeyType.ED25519)).toBe(false);
    });
  });
});
