...
export function MessageInput({
    ...
    const { agent, channel, members, isJoined, refetchJoinStatus, refetchJoinedAgent, refetchChannelMembers, refetchMessageCount, refetchMessages } = useAgentChat();
    ...
    const handleSendMessage = async (message: string) => {
        ...
        if ((message.trim() || mentions.length > 0) && channel && agent) {
            try {
                // 构建包含 mentions 的消息内容
