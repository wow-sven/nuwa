...
| [3](/nip-3.md)   | Custodian-Assisted DID Onboarding Protocol (CADOP)            | Defines a protocol (CADOP) where Identity Providers attest to a user's client-generated DID (e.g., `did:key` from a Passkey) and Sybil resistance level, enabling a Custodian to assist the user in onboarding to their Agent DID while the user maintains full control. | Draft  |
...
Contributions to NIPs are welcome! Please read [NIP-0: NIP Process](/nip-0.md) for details on how to contribute. New proposals should use the [nip-template.md](./nip-template.md).
