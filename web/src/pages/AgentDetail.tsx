...
import { Tabs, DataList, Badge, Button } from "@radix-ui/themes";
...
export function AgentDetail() {
  ...
  const { data: homeChannelResponse, isLoading: isHomeChannelQueryLoading } =
    useRoochClientQuery(
      "executeViewFunction",
      {
      ...
      try {
        if (agentResponse?.data && agentResponse.data.length > 0) {
          ...
          const processedAgent: Agent = {
            ...
            createdAt: Number(agentData.created_at) || Date.now(),
          };
  ...
  useEffect(() => {
    ...
    if (!isHomeChannelQueryLoading) {
      if (homeChannelResponse?.return_values?.[0]?.decoded_value) {
        setHomeChannelId(
          String(homeChannelResponse.return_values[0].decoded_value)
        );
      } else {
  ...
  const handleSaveAgentInfo = async (
    ...
    if (!matchingCap) {
      setUpdateError(
        "You do not have the required authorization to update this agent"
      );
      return;
    }
    ...
    try {
      ...
      if (result.execution_info.status.type !== "executed") {
        throw new Error(
          "Failed to update agent" + JSON.stringify(result.execution_info)
        );
      }
  ...
  const deserializeMemories = (response: any): Memory[] => {
    ...
    try {
      ...
      const cleanHexValue = hexValue.startsWith("0x")
        ? hexValue.slice(2)
        : hexValue;
    ...
    const fetchCurrentUserMemories = async () => {
      ...
      try {
        ...
        const response = await client.executeViewFunction({
          ...
          args: [Args.objectId(agent.id), Args.address(userAddress)],
        });
        ...
        console.error(
          "Failed to fetch agent memories about current user:",
          error
        );
      } finally {
  ...
  const handleAddressSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await client.executeViewFunction({
        target: `${packageId}::agent::get_agent_memories_about_user`,
        args: [Args.objectId(agent.id), Args.address(searchAddress)],
      });
    ...
    <Layout>
      <div className="max-w-4xl mx-auto px-4 py-8">
              ...
              {isEditingName && isUserAuthorized ? (
                <div className="flex-1 flex items-center">
                  ...
                  <div className="flex items-center">
                    <SessionKeyGuard
                      onClick={() => handleSaveAgentInfo("name")}
                    >
                      <button
                        disabled={isSaving}
                        className={`mr-2 text-sm px-3 py-1 rounded ${isSaving ? "bg-blue-300" : "bg-blue-600"} text-white`}
                ...
                <h3 className="text-lg leading-6 font-medium text-gray-900">
                  <span className="text-4xl font-bold">{agent.name}</span>
                  {isUserAuthorized && (
        ...
        <Tabs.Root
          ...
          onValueChange={(value) =>
            setActiveTab(value as "details" | "memories")
          }
          className="mb-4"
        >
          <Tabs.List>
        ...
        {activeTab === "details" ? (
          ...
          >
            <DataList.Item>
              <DataList.Label>Agent ID</DataList.Label>
              <DataList.Value>
            ...
            <DataList.Item>
              <DataList.Label>Agent Address</DataList.Label>
              <DataList.Value>
                <Badge color="blue" variant="soft" radius="full">
                  <a
                    href={`${roochScanBaseUrl}/account/${agent.agentAddress}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    title={`View ${shortenAddress(agent.agentAddress)} on RoochScan`}
                  >
                    {agent.agentAddress}
                  </a>
                </Badge>
            ...
            <DataList.Item>
              ...
              <DataList.Value>@{agent.username}</DataList.Value>
            </DataList.Item>
            {renderEditableField(
              "description",
              "Description",
            ...
            <DataList.Item>
              <DataList.Label>Model Provider</DataList.Label>
              <DataList.Value>
                <Badge color="blue" variant="soft" radius="full">
                  {agent.modelProvider}
                </Badge>
            ...
            <DataList.Item>
              <DataList.Label>Last Active</DataList.Label>
              <DataList.Value>
                {new Date(agent.lastActiveTimestamp).toLocaleString()}
              </DataList.Value>
            </DataList.Item>
            <DataList.Item>
              <DataList.Label>Home Channel</DataList.Label>
              <DataList.Value>
              ...
              <DataList.Value>
                {isLoadingTaskSpecs ? (
                  ...
                  <p className="text-gray-500">
                    No task specifications available
                  </p>
                ) : (
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div className="divide-y divide-gray-200">
              ...
              <div className="px-4 py-5">
                ...
                {isLoadingSelfMemories ? (
                  ...
                  <p className="text-gray-500 text-center py-4">
                    This agent hasn't formed any self memories yet.
                  </p>
                ) : (
              ...
              {searchAddress && (
                <div className="px-4 py-5">
                  <h4 className="text-md font-medium text-gray-900 mb-3">
                    Memories About Address:{" "}
                    <span className="font-mono text-sm">{searchAddress}</span>
                  </h4>
        ...
        <div className="mt-8 p-6 bg-white shadow sm:rounded-lg">
          ...
          <p className="text-gray-600 mb-4">
            Interact with AI agent by sending messages.
          </p>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {isHomeChannelLoading ? (
              ...
              >
                <Button variant="solid">Go to Home Channel</Button>
              </SessionKeyGuard>
            ) : (
