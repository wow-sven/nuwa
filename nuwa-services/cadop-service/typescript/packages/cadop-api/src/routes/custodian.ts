  ...
  if (error instanceof z.ZodError) {
    return {
      ...
      response: createErrorResponse(
        'Validation error',
        CadopErrorCode.VALIDATION_ERROR,
        error.errors
      ),
    };
  }
...
router.get('/status/:recordId', async (req: Request, res: Response) => {
  try {
    ...
    if (!status) {
      return res
        .status(404)
        .json(createErrorResponse('DID creation record not found', CadopErrorCode.NOT_FOUND));
    }
...
router.get('/resolve/:agentDid', async (req: Request, res: Response) => {
  try {
    ...
    if (!didDocument) {
      return res
        .status(404)
        .json(createErrorResponse('Agent DID not found', CadopErrorCode.NOT_FOUND));
    }
