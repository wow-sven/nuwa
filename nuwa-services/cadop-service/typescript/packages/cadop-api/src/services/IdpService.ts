  ...
  private rawPubKeyToCOSE(rawPubKey: Uint8Array, keyType: string): Uint8Array {
    if (keyType === KEY_TYPE.ED25519) {
      ...
      return Buffer.concat([
        header, kty, alg, crv, xHeader, Buffer.from(rawPubKey)
      ]);
    } 
      ...
      try {
        ...
        return Buffer.concat([
          header, kty, alg, crv, 
          xHeader, Buffer.from(x),
          yHeader, Buffer.from(y)
        ]);
      } catch (error) {
        console.error('P-256 key decompression error:', error);
        throw new Error(`P-256 key decompression failed: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    else {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw new Error(`Failed to create COSE key: ${coseError instanceof Error ? coseError.message : String(coseError)}`);
        }
        ...
        throw new Error(`Verification error: ${error instanceof Error ? error.message : JSON.stringify(error)}`);
      }
    } catch (error) {
      console.error('Assertion processing error:', 
        error instanceof Error ? 
        { name: error.name, message: error.message, stack: error.stack } : 
        JSON.stringify(error, null, 2)
      );
