...
export const Descriptions: React.FC<DescriptionsProps> = ({
  ...
  return (
    <div className={cn('w-full', className)} style={style} {...props}>
      {title && <div className="font-medium text-lg mb-4">{title}</div>}
      <div
        className={cn(
      ...
      >
        {layout === 'horizontal' ? (
          <div
            className={cn('grid', gridColsClass, bordered ? 'divide-x divide-y divide-border' : '')}
          >
            {items.map((item, index) => (
              <div
                key={index}
                className={cn('flex flex-col', item.className, bordered ? 'p-3' : 'py-2')}
              ...
              >
                ...
                <div className={cn('text-foreground', item.contentClassName)}>{item.children}</div>
              </div>
            ))}
          </div>
        ) : (
          <div className={cn(bordered ? 'divide-y divide-border' : '')}>
            {items.map((item, index) => (
              <div
                key={index}
                className={cn('grid grid-cols-4', item.className, bordered ? 'p-3' : 'py-2')}
              >
