...
export const DIDCreationStatus: React.FC<DIDCreationStatusComponentProps> = ({
  ...
  const timelineItems = [
    {
      dot: status.status === 'pending' ? <LoadingOutlined /> : <CheckCircleOutlined style={{ color: '#52c41a' }} />,
      children: (
        <div>
    ...
    {
      dot: status.status === 'processing' ? <LoadingOutlined /> : 
           status.status === 'completed' ? <CheckCircleOutlined style={{ color: '#52c41a' }} /> :
    ...
    {
      dot: status.status === 'completed' ? <CheckCircleOutlined style={{ color: '#52c41a' }} /> :
           <ClockCircleOutlined style={{ color: '#d9d9d9' }} />,
      children: (
        <div>
