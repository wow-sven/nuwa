...
export const IPFS_GATEWAY = TARGET === 'local' ? 'http://localhost:5001' : 'https://ipfs.io/ipfs';
export const IPFS_NODE = TARGET === 'local' ? 'localhost' : process.env.IPFS_NODE || (() => { throw new Error('IPFS_NODE is required') })();
export const IPFS_NODE_PORT = TARGET === 'local' ? '5001' : process.env.IPFS_NODE_PORT || (() => { throw new Error('IPFS_NODE_PORT is required') })();
